...
/**
 * JVM-specific implementation of IndexCache with full timestamp support and thread safety.
 * Uses Java's ConcurrentHashMap for thread-safe operations and precise System.currentTimeMillis().
 *
...
actual class IndexCache<T : Element> actual constructor() {
    ...
    /**
     * Thread-safe cache storage using ConcurrentHashMap.
     */
    ...
    @Volatile private var evictions = 0L

    /**
     * Cache entry with precise timestamp.
     */
    ...
    /**
     * Clean up expired entries using precise timestamp comparison.
     */
     ...
     * JVM implementation uses more precise memory estimation.
     */
    actual fun estimateMemoryUsage(): Long {
        // More accurate estimation using Java's object size calculation
        val entryOverhead = 32L // Object overhead per Map entry
        val stringOverhead = 40L // String object overhead
        val setOverhead = 32L // HashSet overhead
        val elementReferenceSize = 8L // Reference size on 64-bit JVM

        var totalSize = 0L
        cache.forEach { (key, entry) ->
            // Key size
            totalSize += stringOverhead + (key.length * 2) // UTF-16 encoding

            // Entry overhead
            totalSize += entryOverhead

            // Result set size
            totalSize += setOverhead + (entry.result.size * elementReferenceSize)

            // Timestamp size (Long)
            totalSize += 8L
        }

        // Add ConcurrentHashMap overhead
        totalSize += 64L + (cache.size * 16L) // Approximate bucket overhead
     ...
     * JVM implementation provides detailed recommendations.
     */
    actual fun getOptimizationRecommendations(): List<String> {
        ...
        val estimatedMemory = estimateMemoryUsage()

        if (hitRatio < 0.5) {
            recommendations.add("Low hit ratio (${(hitRatio * 100).toInt()}%) - consider reviewing query patterns or increasing TTL")
        }
        ...
        if (evictions > hits) {
            recommendations.add("High eviction rate (${evictions} evictions vs ${hits} hits) - consider increasing cache size")
        }

        if (estimatedMemory > 50 * 1024 * 1024) { // 50MB
            recommendations.add("High memory usage (${estimatedMemory / (1024 * 1024)}MB) - consider reducing cache size or TTL")
        }
        ...
        val currentTime = System.currentTimeMillis()
        val expiredCount = cache.values.count { currentTime - it.timestamp > maxAge }
        if (expiredCount > cache.size * 0.2) {
            recommendations.add("Many expired entries ($expiredCount/${cache.size}) - consider more frequent cleanup")
        }
     ...
     * Uses LRU strategy by removing entries with oldest timestamps.
     */
