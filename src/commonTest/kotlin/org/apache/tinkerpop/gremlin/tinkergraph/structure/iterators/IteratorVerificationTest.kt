 ...
 *
 * This test class provides systematic verification of TinkerGraph iterator implementations focusing
 * on core functionality without complex scenarios. It ensures that basic iterator contracts are
 ...
 *
 * These tests serve as foundational validation before more complex iterator scenarios are tested in
 * other test suites. They focus on correctness rather than performance or edge cases.
 *
...
class IteratorVerificationTest :
        StringSpec({
            "basic vertex iteration should work correctly" {
                ...
                allVertices.any { it.value<String>("name") == "alice" }.shouldBeTrue()
                allVertices.any { it.value<String>("name") == "bob" }.shouldBeTrue()
                allVertices.any { it.value<String>("name") == "charlie" }.shouldBeTrue()
            ...
            "basic edge iteration should work correctly" {
                ...
                allEdges shouldHaveSize 2
                allEdges.any { it.label() == "knows" }.shouldBeTrue()
            ...
            "edge traversal should work correctly" {
                ...
                outEdges.any { it.label() == "knows" }.shouldBeTrue()
                outEdges.any { it.label() == "likes" }.shouldBeTrue()
            ...
            "property iteration should work correctly" {
                ...
                val properties =
                        mutableListOf<org.apache.tinkerpop.gremlin.structure.VertexProperty<Any>>()
            ...
            "empty iterators should work correctly" {
                val graph = TinkerGraph.open()
                ...
                vertexIterator.hasNext().shouldBeFalse()
                edgeIterator.hasNext().shouldBeFalse()

                // Test empty traversal
                val vertex = graph.addVertex("name", "lonely")
                val outIterator = vertex.vertices(Direction.OUT)

                outIterator.hasNext().shouldBeFalse()
